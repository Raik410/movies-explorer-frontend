...
const SavedMovies = () => {
    ...
    const handleSearchFilms = (input) => {
        const filterData = allFilms.filter(({nameRU}) => nameRU.toLowerCase().includes(input.toLowerCase()));
        ...
        if (toggle) {
            setFilmsShowed([...shortFilterData]);
    ...
    return (<>
        <SearchForm onSearchFilms={handleSearchFilms}
                    toggle={toggle}
                    setToggle={setToggle}
                    errorText={errorText}
        />
